...
/**
 * @borrows https://github.com/vuejs/vue-cli/blob/dev/packages/%40vue/cli-plugin-pwa/lib/HtmlPwaPlugin.js
 */

const HtmlWebpackPlugin = require('html-webpack-plugin')

const ID = 'vue3-boilerplate:html-pwa-plugin'
...
module.exports = class HtmlPwaPlugin {
  ...
  apply(compiler) {
    ...
    if (!isHrefAbsoluteUrl(this.options.manifestPath)) {
      const { name, themeColor, manifestPath, manifestOptions } = this.options
      const publicOptions = {
        name,
        short_name: name,
      ...
      compiler.hooks.compilation.tap(ID, (compilation) => {
        compilation.hooks.processAssets.tap(
          ...
          (assets) => {
            assets[manifestPath] = manifestAsset
          }
        )
      })
    }
